...
fn create_thread(ctx: &mut CallCtx<'_>) -> Result<DispatchOutcome, KernelError> {
    ...
    // Same guard margin as the main thread's stack (see the comment in
    // pocket-kernel's `Emulator::new` / `map_region` call) — a worker
    // thread can be the one whose stack top gets probed just as easily
